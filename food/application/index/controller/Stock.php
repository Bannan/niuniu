...
//print_r($arr);die;
    ...
    public function setting()
    {
        ...
        {
            case 'update1':
                echo json_encode(["status"=>$db1->where(["id"=>input("id")])->update(input("post.")) ? 1 : 0]);die;
                break;
            case 'add1':
                echo json_encode(["status"=>$db1->where(["id"=>input("id")])->insert(input("post.")) ? 1 : 0]);die;
                break;
            case 'edit1':
                echo json_encode($db1->where(["id"=>input("id")])->find());die;
                break;
            case 'update2':
                echo json_encode(["status"=>$db2->where(["id"=>input("id")])->update(input("post.")) ? 1 : 0]);die;
                break;
            case 'add2':
                echo json_encode(["status"=>$db2->where(["id"=>input("id")])->insert(input("post.")) ? 1 : 0]);die;
                break;
            case 'edit2':
                echo json_encode($db2->where(["id"=>input("id")])->find());die;
                break;
        }
        //print_r($db1->select());die;
